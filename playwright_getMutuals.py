...
MUTUALS_FILE = "abigail_mutuals.csv"
GRAPH_FILE = "abigail_mutuals_graph.json"
...
def scrape_following(page, target_user, mutuals):
    ...
    while stable_rounds < STOP_AFTER_UNCHANGED_ROUNDS:
        # Scroll the modal
    ...
    print(f"✅ Found {len(usernames)} mutual's usernames {target_user} is following")
    if len(all_usernames) <= 12:
        raise Exception(f"Found too few usernames ({len(all_usernames)}) for {target_user}. Check the profile or your connection.")
